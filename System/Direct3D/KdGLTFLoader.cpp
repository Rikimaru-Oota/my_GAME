...
std::shared_ptr<KdGLTFModel> KdLoadGLTFModel(const std::string& path)
{
	...
	//----------------------------------
	{
		...
		for (UINT matei = 0; matei < destModel->Materials.size(); matei++)
		{
			...
			// 基本色
			destMaterial.BaseColorTexture = GetTextureFilename(srcMaterial.pbrMetallicRoughness.baseColorTexture.index);
			...
			// 金属性、粗さ
			destMaterial.MetallicRoughnessTexture = GetTextureFilename(srcMaterial.pbrMetallicRoughness.metallicRoughnessTexture.index);
			...
			// エミッシブ
			destMaterial.EmissiveTexture = GetTextureFilename(srcMaterial.emissiveTexture.index);
	...
	{
		destModel->Nodes[nodei] = std::make_shared<KdGLTFNode>();
	}
	...
	for (UINT nodei = 0; nodei < destModel->Nodes.size(); nodei++)
	{
		...
		// Index
		destNode->Offset = nodei;	// 後にリストの先頭にRootノードを加えるので、+1で記憶

		// 子への参照
		for (auto&& childIdx : model.nodes[nodei].children)
		{
			destNode->Children.push_back(destModel->Nodes[childIdx]);
		}
	...
	// ルートノードのみの参照リスト
	//----------------------------------
	for (auto&& n : model.scenes[0].nodes)
	{
		destModel->RootNodes.push_back(destModel->Nodes[n]);
	}
	...
	{
		// 行列計算用 再帰関数
		std::function<void(std::shared_ptr<KdGLTFNode>, const Math::Matrix*)> rec = [&rec](std::shared_ptr<KdGLTFNode> node, const Math::Matrix* parentMat)
		{
			if (parentMat) {
			...
			{
				rec(child, &node->WorldTransform);
			}
		};

		// 親子関係から行列を作成
		for (auto&& node : destModel->RootNodes)
		{
			rec(node, nullptr);
		}
	...
	for (UINT nodei = 0; nodei < destModel->Nodes.size(); nodei++)
	{
		...
		for (size_t pri = 0; pri < model.meshes[msi].primitives.size(); pri++)
		{
			...
			// 頂点バッファ
			{
				...
				for (UINT vi = 0; vi < posGetter.GetAccessor()->count; vi++) {
					...
					ver.Pos.z = posGetter.GetValue_Float(vi * 3 + 2);
				}
				...
				if (srcPrimitive.attributes.count("NORMAL") > 0)
				{
					...
					for (UINT vi = 0; vi < destPrimitive->Vertices.size(); vi++) {
						...
						nor.z = normalGetter.GetValue_Float(vi * 3 + 2);
					}
				}
			...
			// インデックスバッファ
			{
				...
				for (UINT di = 0; di < destPrimitive->Faces.size(); di++)
				{
					...
					destPrimitive->Faces[di].Idx[1] = (UINT)indexGetter.GetValue_Int(di * 3 + 1);
					destPrimitive->Faces[di].Idx[2] = (UINT)indexGetter.GetValue_Int(di * 3 + 2);
		...
		for (UINT pi = 0; pi < tempPrimitives.size(); pi++)
		{
			...
			if (prim->Vertices.size() >= 1) {
				...
				memcpy(&destNode->Mesh.Vertices[st], &prim->Vertices[0], prim->Vertices.size() * sizeof(MeshVertex));
			}
	...
	for (UINT ani = 0; ani < model.animations.size(); ani++)
	{
		...
		// 
		std::vector<std::shared_ptr<AnimationData::Node>> tempNodes;
		...
		for (const auto& channel : srcAni.channels)
		{
			...
			{
				destAnimNode = std::make_shared<AnimationData::Node>();
			...
			if (channel.target_path == "translation")
			{
				...
				{
					AnimationData::KeyVector3 v;
					// 時間
					v.Time = timeGetter.GetValue_Float(ki);
			...
			else if (channel.target_path == "rotation")
			{
				...
				for (UINT ki = 0; ki < timeGetter.GetAccessor()->count; ki++)
				{
					AnimationData::KeyQuaternion q;
					// 時間
					q.Time = timeGetter.GetValue_Float(ki);
					...
					if (sampler.interpolation == "STEP")
					{
						q.Quat.x = valueGetter.GetValue_Float(ki * 4 + 0);
						q.Quat.y = valueGetter.GetValue_Float(ki * 4 + 1);
					...
					{
						q.PrevTangent.x = valueGetter.GetValue_Float(ki * 12 + 0);
						q.PrevTangent.y = valueGetter.GetValue_Float(ki * 12 + 1);
						q.PrevTangent.z = valueGetter.GetValue_Float(ki * 12 + 2);
						q.PrevTangent.w = valueGetter.GetValue_Float(ki * 12 + 3);

						q.Quat.x = valueGetter.GetValue_Float(ki * 12 + 4);
						q.Quat.y = valueGetter.GetValue_Float(ki * 12 + 5);
						q.Quat.z = valueGetter.GetValue_Float(ki * 12 + 6);
						q.Quat.w = valueGetter.GetValue_Float(ki * 12 + 7);

						q.NextTangent.x = valueGetter.GetValue_Float(ki * 12 + 8);
						q.NextTangent.y = valueGetter.GetValue_Float(ki * 12 + 9);
