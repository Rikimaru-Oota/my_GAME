...
public:
	...
	// データのポインタを取得
	//  ※アクセスが終わったら必ずUnmap()を実行すること
	//  ※Defaultバッファ	… Mapできません　nullptrが返る
	//  ※Dynamicバッファ	… D3D11_MAP_WRITE_DISCARD、D3D11_MAP_WRITE_NO_OVERWRITEでMap可能
	//  ※Stagingバッファ	… 全てのMapTypeでMap可能(だと思います)
	//  ※詳細な仕様は https://msdn.microsoft.com/ja-jp/library/ee416245(v=vs.85).aspx を参照
	// ・mapType	… D3D11_MAPを指定
	// 戻り値	… データの先頭アドレス　失敗時はnullptr
	...
	//  ※DXGIフォーマットの互換性が必要です。
	// ・srcBuffer		… コピー元バッファ
	void CopyFrom(KdBuffer& srcBuffer);
